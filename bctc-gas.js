...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://www.pvgas.com.vn/quan-he-co-%C4%91ong/tai-lieu-co-%C4%91ong', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('.edn_simpleList').find('a').each((_, el) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.GAS);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.GAS);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của GAS ::: ${name}`);
        })
      );
