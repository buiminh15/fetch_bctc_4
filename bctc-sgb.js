...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://www.saigonbank.com.vn/vi/quan-he-co-dong/Bao-cao-hoat-dong', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('.col-lg-8.col-md-8.col-sm-12.col-xs-12').find('a').each((_, el) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.SGB);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.SGB);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của SGB ::: ${name}`);
        })
      );
