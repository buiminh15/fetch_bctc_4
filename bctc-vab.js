...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://vietabank.com.vn/nha-dau-tu/bao-cao-tai-chinh.html', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('.wrap-list-box').find('P').each((_, el) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.VAB);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.VAB);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của VAB ::: ${name}`);
        })
      );
