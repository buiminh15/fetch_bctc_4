...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://www.abs.vn/co-dong/bao-cao-tai-chinh/', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.ABW);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.ABW);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của ABW ::: ${name}`);
        })
      );
