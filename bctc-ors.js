...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://tpbs.com.vn/vi/thong-tin-tps/quan-he-co-dong/bao-cao-tai-chinh', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.ORS);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.ORS);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của ORS ::: ${name}`);
        })
      );
