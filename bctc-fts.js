...
async function fetchAndExtractData() {
  try {
    const response = await axios.get(
      'https://fpts.com.vn/editor/Surface/news/GetNews?page=1&pageSize=10&groupid=105',
      {
        headers: {
    ...
    const newNames = await filterNewNames(names, COMPANIES.FTS);
    console.log('📢 [bctc-geg.js:44]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.FTS);

      // Gửi thông báo Telegram cho từng báo cáo mới;
      await Promise.all(
        newNames.map(name =>
          sendTelegramNotification(`Báo cáo tài chính của FTS::: ${name}`)
        )
      );
