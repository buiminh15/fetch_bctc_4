...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://kienlongbank.com/bao-cao-tai-chinh', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('.canhcam-kl-1').find('h5.title').each((_, el) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.KLB);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.KLB);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của KLB ::: ${name}`);
        })
      );
