...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://www.shs.com.vn/Periodical.aspx', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('#ctl00_webPartManager_wp1235669633_wp677033281_cbHeadNews').find('a').each((_, el) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.SHS);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.SHS);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của SHS ::: ${name}`);
        })
      );
