...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://www.vietcap.com.vn/quan-he-co-dong/bao-cao-tai-chinh', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('span.title.max-2-line').each((_, el) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.VCI);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.VCI);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của VCI ::: ${name}`);
        })
      );
