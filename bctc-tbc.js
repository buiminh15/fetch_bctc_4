...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://thacba.vn/c3/pages-t/Bao-cao-tai-chinh-8-138.aspx', {
      headers: {
        'accept': 'text/html',
    ...
    const newNames = await filterNewNames(names, COMPANIES.TBC);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.TBC);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của TBC ::: ${name}`);
        })
      );
