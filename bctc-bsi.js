...
async function fetchAndExtractData() {
  try {
    const response = await axios.get('https://www.bsc.com.vn/quan-he-co-dong/', {
      headers: {
        'accept': 'text/html',
    ...
    const names = [];
    $('#bao-cao-tai-chinh').find('p.main_title').each((_, el) => {
    ...
    const newNames = await filterNewNames(names, COMPANIES.BSI);
    console.log('📢 [bctc-cdn.js:46]', newNames);
    if (newNames.length) {
      await insertBCTC(newNames, COMPANIES.BSI);

      // Gửi thông báo Telegram cho từng báo cáo mới
      await Promise.all(
        newNames.map(name => {
          return sendTelegramNotification(`Báo cáo tài chính của BSI ::: ${name}`);
        })
      );
